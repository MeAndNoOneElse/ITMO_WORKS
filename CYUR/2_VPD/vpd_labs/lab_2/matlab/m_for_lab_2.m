...
plot(omega_steady_rad, U_ctrl_valid, 'bo-', 'MarkerSize', 5, 'LineWidth', 1.5, 'MarkerFaceColor', 'b'); hold on;

% Диапазон от минимальной до максимальной скорости (включая отрицательные)
omega_fit = linspace(min(omega_steady_rad), max(omega_steady_rad), 100);
U_fit = k_e_static * omega_fit;   % теперь прямая проходит через весь диапазон
